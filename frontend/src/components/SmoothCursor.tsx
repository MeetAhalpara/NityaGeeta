...
export type CursorMode = "default" | "text" | "pointer" | "loading";
...
/* Official Magic UI Terracotta Cursor SVG */
...
/* Sleek & Compact Text I-Beam Caret SVG */
...
/* Button / Link Hover Pointer SVG with Pulse Aura */
const PointerCursorSVG: FC = () => (
  <div className="relative flex items-center justify-center">
    <div className="absolute w-10 h-10 rounded-full bg-[#C25E38]/25 animate-ping" />
    <DefaultCursorSVG />
  </div>
);

/* Loading / Waiting State Spinning Wheel SVG */
const LoadingCursorSVG: FC = () => (
...
export function SmoothCursor({
  ...
  useEffect(() => {
    ...
    const detectCursorMode = (target: Element | null) => {
      if (!target) {
        setMode("default");
        return;
      }
      try {
        const isLoading =
          target.closest("[data-loading='true'], .loading, [aria-busy='true']") ||
          document.body.classList.contains("cursor-loading");
        if (isLoading) {
          setMode("loading");
          return;
        }
        ...
        const isInteractive = target.closest(
          "button, a, select, option, [role='button'], .cursor-pointer"
        );
        if (isInteractive) {
          setMode("pointer");
          return;
  ...
  return (
    <motion.div
      style={{
        ...
        translateX: "-50%",
        translateY: "-50%",
        rotate: mode === "text" || mode === "loading" ? 0 : rotation,
        scale: scale,
        zIndex: 2147483647,
        pointerEvents: "none",
