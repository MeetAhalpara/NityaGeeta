  ...
  return createPortal(
    ...
    >
      ...
      <motion.div
        ...
        className={cn(
          "pointer-events-auto absolute inset-y-0 flex h-dvh w-[var(--sidebar-width-mobile)] max-w-[88vw] flex-col overflow-hidden",
    ...
    return (
      ...
      >
        <motion.div
          ...
          className={cn(
            "sticky top-0 flex h-svh w-full flex-col overflow-hidden bg-[#F4EFE6] dark:bg-[#12100F]",
            variant === "sidebar" &&
              (side === "left" ? "border-[#E6DDD0] dark:border-[#2D2825] border-r" : "border-[#E6DDD0] dark:border-[#2D2825] border-l"),
            variant === "floating" &&
...
>(function AnimatedSidebarTrigger(
  { className, onClick, type = "button", showLabel = false, children, ...props },
  forwardedRef,
) {
  const context = useAnimatedSidebar();
...
>(function AnimatedSidebarFooter({ className, ...props }, forwardedRef) {
  return (
    <div
      ...
      className={cn(
        "flex shrink-0 flex-col gap-2 border-[#E6DDD0] dark:border-[#2D2825] border-t p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))]",
        className,
      )}
    />
  );
...
>(function AnimatedSidebarMenu(
  ...
  return (
    <SharedLayoutBg
      ...
      as="ul"
      inset={0}
      pillClassName="rounded-xl bg-transparent"
    ...
    <AnimatePresence initial={false} mode="popLayout">
      {open && !panel.collapsed ? (
        <motion.ul
          ...
          className={cn(
            "relative mt-1 ml-5 flex min-w-0 flex-col gap-0.5 border-[#E6DDD0] dark:border-[#2D2825] border-l pl-3",
            className,
          )}
        >
          {children}
    ...
    <>
      {isActive && !panel.collapsed ? (
        <motion.span
          ...
          className="absolute inset-0 rounded-xl border-l-2 border-[#C25E38] dark:border-[#E06D43]"
        />
      ) : null}
      {icon ? (
        <span
